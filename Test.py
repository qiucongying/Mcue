...
with tf.name_scope('unet'):
    model=UNet().create_model(img_shape=img_shape+[num_channels], num_class=num_class)
    img=model.input
...
with sess.as_default():    
    ...
    for imgname in imgs:
        ...
        if height<ROISize: 
            src=cv2.resize(src,(int(width*ROISize/height), ROISize), interpolation=cv2.INTER_NEAREST)
        if width<ROISize:    
            src=cv2.resize(src,(ROISize, int(height*ROISize/width)), interpolation=cv2.INTER_NEAREST)
        ...
        for i in range(numc):
            for j in range(numr):
                ROI=GetROI(src,i,j)
                ROI= cv2.resize(ROI,(imHeight,imWidth), interpolation=cv2.INTER_NEAREST)
                Cellx=cut(i*ROISize,0,imw-ROISize-1)
                Celly=cut(j*ROISize,0,imh-ROISize-1)
                if len(ROI.shape)==2:
                    ROI=ROI[None,:,:,None]
    ...
    #            cv2.imshow("ROI",ROI[0])               
                Mfe=1/(np.exp((pred_logits[:,:,0]-pred_logits[:,:,1])*0.8)+1)
                Mfe=boundarydiscard(Mfe,1,200)
                Mfe=cv2.resize(Mfe,(ROISize,ROISize))
                ROI=Salimg[Celly:Celly+ROISize,Cellx:Cellx+ROISize]
                cat2=np.concatenate((Mfe[:,:,None],ROI[:,:,None]),axis=2)
                ROI=np.max(cat2,axis=2)
                Salimg[Celly:Celly+ROISize,Cellx:Cellx+ROISize]=ROI
                Salimg[Celly:Celly+ROISize,Cellx:Cellx+ROISize]=Mfe+Salimg[Celly:Celly+ROISize,Cellx:Cellx+ROISize]
